...
com <- filter(com, nation == "scotland")
...
```{r define_stats}
stats <- c("age-five-year", "ethnic-group",
           "ethnic-group-grouped", "religion")
...
read_com_data <- function(x){
  ...
  census_file <-
    here("data", "com-stats",
         str_c("scottish-com_", x, "-scotland-2021.csv"))
...
national_data <-
  map(national_data, \(x) mutate(x, nation = "scotland"))
...
saveRDS(all_data,
        file.path(save_dir, "scotland_all-com-data_no-relative-counts.RDS"))

vroom_write(all_data, 
      file.path(save_dir, "scotland_all-com-data_no-relative-counts.csv"),
      delim = ",")
```
